    ...
    return (
        <>
            <div className='log-in-page form-container'>
                ...
                <LogInAndSignUpForm setIsLogIn={setIsLogIn} onLogInBtnClick={loginUser} onSignUpBtnClick={registerUser} />
            </div>
        </>
    );
}

function LogInAndSignUpForm({ setIsLogIn, onLogInBtnClick = async () => { }, onSignUpBtnClick = async () => { } }) {
        ...
        <div className='card p-4 bg-glass bg-light-white'>
            ...
            <form className='form card-body d-flex flex-column' style={{ gap: (!isSignInState ? '1rem' : '0.5rem') }}>
                {!isSignInState && <>
                    ...
                    <button className='btn btn-primary' type="submit" ref={loginBtn} onClick={async e => {
                        e.preventDefault();
                        await onLogInBtnClick({ email, password });
                    }}>Log In</button>
                    <div>
...
function SignUpForm({ setIsSignInState, onSignUpBtnClick }) {
    const [email, setEmail] = useState('');
    const [password, setPassword] = useState('');
    ...
    return (
        <>
            ...
            <button type="submit" className='btn btn-primary' onClick={async e => {
                e.preventDefault()
                await onSignUpBtnClick({ email, password, confirmPassword, firstName, lastName })
